...
data forecast;
set orion.growth;
total_year1 = Total_Employees * (1+Increase);
year = 1;
...
title "Employees in United States";
proc print data = usa;
run;
...
when ("Stock and Shipping") output work.stock;
...
*data usa (drop = street_ID) *affects the output data;
*set orion.employee_addresses(drop=Street_ID) - affects input data;
* proc print data = usa (drop = country) affects proc output;
...
data australia;
set orion.employee_addresses (firstobs=20);
if country = "AU" then output;
run;
...
data work.salesinfo (keep = Employee_ID Job_Title Manager_ID) work.execinfo (keep = Employee_ID Job_Title);
set orion.employee_organization;
...
proc print data=work.salesinfo (obs=6);
run;
title;
...
input @1 cust_type 4. /*1st value as numeric value - column 1;*/
	  @5 offer_dt mmddyy8.; *column 5;
run;

* +n move to column left or right;
...
* multiple input statements for multiple lines;

* / moves the input pointer forward;

* # absolute no of lines which you want to move forward. put # sign before instruction were reading the values.;
...
input SaleID $4. @6 Location $3. @; * @ holds the input statement from moving to the next record;
...
infile "&path/sales.dat";
input SaleID $4. @6 Location $3. @; * @ holds the input statement from moving to the next record;
if Location = "EUR"; * subsetting if statement;
input 	@1 SALESID $4
		@10 SalesDate date9.
		@20 Amount commax7.;
run;
...
*lowcase and upcase for the lower and upper case respectively;
...
data labels;
set orion.contacts;
keep FullName Address1 Address2;
...
* trim removes the trailing blank while strip removes the blank both sides.cats strips leading and trailing blanks while catt removes the trailing blank;
...
*tranward - used to correct the missplled word;
data correct;
set orion.clean_up;
if find(product, "Mittens", "I")>0 then do;
substr(Product_ID,9,1) = 5;
Product = tranwrd (Product, "Luci ","Lucky ");
Product = tranwrd (Product, "mittens","Mittens");
end;
...
data names;
   ...
   if Gender="F" then New_Name=CATX
      (' ','Ms.',FMNames,Last);
   else if Gender="M" then New_Name=CATX
           (' ','Mr.',FMNames,Last);
...
data work.silver work.gold work.platinum;
   ...
   if find(Customer_ID,'Silver','I') > 0 then
      output work.silver;
   else if find(Customer_ID,'Gold','I') > 0
        then output work.gold;
   else if find(Customer_ID,'Platinum','I') > 0
        then output work.platinum;
...
keep Employee_ID Total AvgQT NumQT Ceil_QT Floor_QT Int_QT;
total = sum(of Qtr1-Qtr4);
avgQt = round(mean(of Qtr1-Qtr4));
NumQt = n (of Qtr1-Qtr4);
Ceil_QT = Ceil(AvgQT);
Floor_QT = Floor(AvgQt);
Int_QT = Int(AvgQt);
run;
...
set orion.convert;
EmpId = Id + 11000;
Bonus = input(GrossPay, Comma6.) * .10; /* Explicit conversion for Character to Numeric */
run; 

/* Explicit conversion from Numeric to Character */
...
create table direct_report as
select Employee_ID, Job_Title, Salary from orion.sales_mgmt where salary>95000;
...
select sales_mgmt.Employee_ID, Employee_Name, Job_Title, Salary from Orion.sales_mgmt, orion.employee_addresses where sales_mgmt.Employee_ID = employee_addresses.Employee_ID;
...
proc print data=future_expenses;
   format Wages Retire Medical Total_Cost
          comma14.2;
   var Year Wages Retire Medical Total_Cost;
run;

data invest;
do year = 1 to 30 until (Capital > 1000000);
Capital + 5000;
Capital + (Capital* .045);
end;
run;
...
data invest2;
do year = 1 to 30 while (Capital<=1000000);
Capital+5000;
Capital+(Capital*.045);
end;
run;
...
array contrib{4} Qtr1-Qtr4;
array diff{4};
array goal{4} _temporary_ (10,20,20,15); /* instead of dropping the temp variables */
do i = 1 to 4;
...
*footnote2 "on the Win System using 9.3";
...
*create a macro variable using %let and reference using &;

%let year = 2012;
%let department = Sales and MArketing;
%let total = 12;
%let City = Cary, NC;
...
data country_info;
keep start label FmtName;
retain FmtName '$Country'; /* initialize the variable and retains the values */
set orion.country(rename = (Country = Start Country_Name = label)); /* rename on input */
run;

proc format library=abc cntlin = country_info;
run;
...
* FMTSEARCH will search for the formats in the library and work folder;

options fmtsearch = (orion work library); * this will search for the formats in three folders - orion, work and library;

options NOFMTERR; /* replace the missing formats with w. or $w. formats and issues the note */
...
value $extra /* $extra replaces the blank with unknown */
' ' = 'unknown'
other = [$country30.]; /* the default length is 40 */
run; 
...
/* easy to forget the spelling */

proc catalog catalog = orion.formats; /*proc catalog will manage all the stored formats */
contents;
run;
/* to document the formats */
proc format library = orion.fmtlib; /* return $sign, includes the length of the longest label, number of values, version and date and time of creation */
select $country;
run;

*Use the FMTLIB keyword in the PROC FORMAT statement to document the formats in a catalog. 
You can add the SELECT and EXCLUDE statements to process specific formats rather than the entire catalog.;

proc format library = orion cntlout = countrtfmt;
select $country;
run;
...
proc sort data = orion.order_fact out = work.orders_2007;
by customer_id;
where year(Order_date) = 2007;
run;

data custord;
merge orion.customer (in = cust) work.orders_2007 (in = order);
by customer_id;
...
data revenue
   ...
   by Product_ID;
   if InProduct and InOrders then
     do; 
...
noorders (keep = Customer_names Birth_date);
...
bonus. 'Supplier$'n (in = s rename = (SupplyID=Supplier Quantity = Minimum)); /* will rename the quantity to minimum in supplier dataset*/
by Supplier;
if c= 1 and s = 1;
run;
